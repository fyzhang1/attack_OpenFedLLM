...
# edit begin
'''
--model_name_or_path pretrained_model/ehartford/Wizard-Vicuna-7B-Uncensored/
...
--local_data_dir /root/autodl-tmp/0722_code_OpenFedLLM/dataset/
'''
# edit end
...
attack_file_path = "/root/autodl-tmp/0722_code_OpenFedLLM/dataset/Attack_dataset/untargeted/untargeted_all_dataset.csv"
local_datasets = load.load_attack_data(attack_file_path)
...
for round in tqdm(range(fed_args.num_rounds)):
    ...
    for client in range(fed_args.num_clients):
        ...
        new_lr = cosine_learning_rate(round, fed_args.num_rounds, script_args.learning_rate,
                                      1e-5)  # manually schedule the learning rate
    ...
    global_dict, global_auxiliary = global_aggregate(fed_args, global_dict, local_dict_list, sample_num_list,
    ...
    # ===== Save the model =====
    # edit begin
    fed_args.save_model_freq = 50
    # edit end
    # ===== 每十轮保存一次 =====
    if (round+1) % 10 == 0:
        np.save(os.path.join(script_args.output_dir, f"global_model_round_{round+1}.npy"), global_dict)
        for client_id in range(fed_args.num_clients):
            np.save(os.path.join(script_args.output_dir, f"local_model_client_{client_id}_round_{round+1}.npy"), local_dict_list[client_id])
